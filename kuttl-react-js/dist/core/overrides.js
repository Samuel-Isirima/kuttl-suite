 ...
 */
export function buildOverrideIndex(patches) {
    const index = new Map();
    function get(uid) {
        if (!index.has(uid))
            index.set(uid, {});
        ...
        switch (patch.op) {
            ...
            case "addClass": {
                ...
                if (override.classesToRemove) {
                    override.classesToRemove = override.classesToRemove.filter((c) => !patch.payload.classes.includes(c));
                }
                break;
            }
            case "removeClass": {
                ...
                if (override.classesToAdd) {
                    override.classesToAdd = override.classesToAdd.filter((c) => !patch.payload.classes.includes(c));
                }
                break;
            }
            case "setText":
