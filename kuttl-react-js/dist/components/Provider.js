...
// Context
// ─────────────────────────────────────────────
const CuttlefishContext = React.createContext(null);
CuttlefishContext.displayName = "CuttlefishContext";
// ─────────────────────────────────────────────
// Provider
// ─────────────────────────────────────────────
export function CuttlefishProvider({ children, ai, persistKey, debug = true, uidAttribute = "data-uid", }) {
    ...
    const overrideIndex = React.useMemo(() => buildOverrideIndex(patches), [patches]);
    ...
    const indexRef = React.useRef(overrideIndex);
    ...
    React.useEffect(() => {
        ...
        index.forEach((override, uid) => {
            const el = document.querySelector(`[${uidAttribute}="${uid}"]`);
            if (!el)
                return;
            // ── Hide / Show ──
    ...
    function patch(input) {
        ...
        addPatches(stamped);
        if (debug)
            console.log("[Cuttlefish] patch", stamped);
    }
    function undo() {
    ...
    async function prompt(text) {
        if (!ai)
            throw new Error("[Cuttlefish] prompt() called without ai config in CuttlefishProvider");
        const root = document.body; // serialiser walks from body
        const { result, status } = await sendAIPrompt(text, ai, root, uidAttribute, overrideIndex, selectedUid, ai.systemPromptExtra);
        if (result.patches.length > 0)
            addPatches(result.patches);
        if (debug) {
    ...
    const onMouseOver = React.useCallback((e) => {
        if (!selectActive.current)
            return;
        const target = e.target.closest(`[${uidAttribute}]`);
        if (target) {
            e.stopPropagation();
    ...
    const onClick = React.useCallback((e) => {
        ...
        if (overlayRef.current) {
            ...
            setTimeout(() => { if (overlayRef.current)
                overlayRef.current.style.background = "rgba(34,197,94,0.06)"; }, 200);
        }
    }, [uidAttribute]);
...
// ─────────────────────────────────────────────
export function useCuttlefishContext() {
    const ctx = React.useContext(CuttlefishContext);
    if (!ctx)
        throw new Error("[Cuttlefish] useCuttlefishContext must be used inside <CuttlefishProvider>");
    return ctx;
}
// ─────────────────────────────────────────────
// Helpers
