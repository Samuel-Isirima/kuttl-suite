 ...
 */
export function serializeReactTree(
  root: Element,
  uidAttr: string,
  ...
  if (inViewport) result["inViewport"]  = true;
...
async function callAnthropic(system: string, user: string, cfg: AIProviderConfig): Promise<string> {
  ...
  if (!block) throw new Error("Anthropic: no text block");
  return block.text;
}

async function callOpenAI(system: string, user: string, cfg: AIProviderConfig): Promise<string> {
...
async function callGemini(system: string, user: string, cfg: AIProviderConfig): Promise<string> {
  ...
  const data = await r.json() as { candidates: Array<{ content: { parts: Array<{ text: string }> }; finishReason: string }> };
  const c = data.candidates[0];
  if (!c) throw new Error("Gemini: no candidates");
  if (c.finishReason === "SAFETY") throw new Error("Gemini: blocked by safety filter");
...
function parseResponse(raw: string): LLMResponse {
  ...
  if (Array.isArray(parsed)) return { type: "patches", patches: parsed };
  if (typeof parsed !== "object" || !parsed) throw new Error("Not an object");
  const obj = parsed as Record<string, unknown>;
  if (obj["type"] === "clarification") return { type: "clarification", question: String(obj["question"]) };
  if (obj["type"] === "patches" && Array.isArray(obj["patches"])) return { type: "patches", patches: obj["patches"] };
  if (typeof obj["op"] === "string") return { type: "patches", patches: [obj] };
  throw new Error(`Unknown response type: ${String(obj["type"])}`);
}

function stampPatch(raw: unknown): Patch {
  ...
  if (!validOps.includes(String(obj["op"]))) throw new Error(`Invalid op: ${String(obj["op"])}`);
  if (!obj["target"]) throw new Error("Missing target");
  return { ...(obj as Omit<Patch,"id"|"timestamp"|"source">), id: generatePatchId(), timestamp: Date.now(), source: "ai" } as Patch;
}
...
export async function sendAIPrompt(
  ...
  try {
    switch (cfg.provider) {
      case "anthropic": raw = await callAnthropic(systemPrompt, userMessage, cfg); break;
      case "openai":    raw = await callOpenAI(systemPrompt, userMessage, cfg); break;
      case "gemini":    raw = await callGemini(systemPrompt, userMessage, cfg); break;
    }
  } catch (err) {
  ...
  for (const p of parsed.patches) {
    try { patches.push(stampPatch(p)); }
    catch (e) { warnings.push(`Skipped: ${e instanceof Error ? e.message : String(e)}`); }
  }
