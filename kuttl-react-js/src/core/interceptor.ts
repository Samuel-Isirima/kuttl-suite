...
const ReactMutable = _React as unknown as { createElement: (...args: any[]) => any };

// ─────────────────────────────────────────────
// Types
// ─────────────────────────────────────────────

type CreateElementFn = typeof _React.createElement;

// ─────────────────────────────────────────────
// Apply overrides to a React element's props
// ─────────────────────────────────────────────

function applyOverridesToProps(
  ...
  // ── Hide ──
  if (override.hidden) {
    const existing = (next["style"] as _React.CSSProperties | undefined) ?? {};
  ...
  if (override.classesToAdd?.length || override.classesToRemove?.length) {
    const current = ((next["className"] as string | undefined) ?? "").split(" ").filter(Boolean);
...
function applyReorderToChildren(
  children: unknown[],
  uid: string,
  uidAttr: string,
  index: OverrideIndex
  ...
  const override = index.get(uid);
  if (!override?.order?.length) return children;
  ...
  for (const child of children) {
    if (_React.isValidElement(child)) {
      const childUid = (child.props as Record<string, unknown>)[uidAttr] as string | undefined;
      if (childUid) childMap.set(childUid, child);
    }
  }
  ...
  const unlisted  = children.filter((c) => {
    if (!_React.isValidElement(c)) return true;
    const cUid = (c.props as Record<string, unknown>)[uidAttr] as string | undefined;
...
let _original: CreateElementFn | null = null;
...
export function installInterceptor(
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ReactMutable.createElement = function patchedCreateElement(...args: any[]): any {
    const [type, props, ...children] = args as [
      Parameters<CreateElementFn>[0],
      Record<string, unknown> | null | undefined,
      ...unknown[]
    ];

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const original = _original as any;

    if (typeof type !== "string" || !props) {
      return original(type, props as never, ...children);
    }

    const uid = props[uidAttr] as string | undefined;
    if (!uid) {
      return original(type, props as never, ...children);
    }
    ...
    const flatChildren      = (children as unknown[]).flat();
    const reorderedChildren = applyReorderToChildren(flatChildren, uid, uidAttr, index);

    return original(type, patchedProps as never, ...reorderedChildren);
  };
}
 ...
 */
function kebabToCamel(styles: Record<string, string>): _React.CSSProperties {
  const result: Record<string, string> = {};
  for (const [key, value] of Object.entries(styles)) {
    const camel = key.replace(/-([a-z])/g, (_, letter: string) => letter.toUpperCase());
    result[camel] = value;
  }
  return result as _React.CSSProperties;
}