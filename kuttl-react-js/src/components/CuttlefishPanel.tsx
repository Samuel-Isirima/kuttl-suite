...
export function CuttlefishPanel({
  defaultProvider = "anthropic",
  defaultApiKey   = "",
}: CuttlefishPanelProps): React.ReactElement | null {
  const ctx = useCuttlefishContext();
  const { sendPrompt, status, isLoading } = useCuttlefishAI();

  const [open,       setOpen]       = React.useState(false);
  const [expanded,   setExpanded]   = React.useState(false);
  const [activeTab,  setActiveTab]  = React.useState<"patches" | "prompts">("patches");
  const [provider,   setProvider]   = React.useState<AIProvider>(defaultProvider);
  const [apiKey,     setApiKey]     = React.useState(defaultApiKey);
  const [promptText, setPromptText] = React.useState("");
  const [history,    setHistory]    = React.useState<PromptEntry[]>([]);
  ...
  const statusText = React.useMemo(() => {
    if (status.state === "loading")              return { text: "Thinking…",           cls: "ctf-status-loading" };
    if (status.state === "error")                return { text: status.message,         cls: "ctf-status-err" };
    if (status.state === "clarification_needed") return { text: status.question,        cls: "ctf-status-clarify" };
  ...
  async function handleSend() {
    ...
    setHistory((h) => [{ text, timestamp: Date.now(), patchCount: after - before, error: status.state === "error" ? (status as { message: string }).message : null }, ...h]);
  }
  ...
  return React.createElement(React.Fragment, null,
    ...
    React.createElement("div", {
      ...
      // Header
      React.createElement("div", { className: "ctf-header" },
        React.createElement("span", { className: "ctf-title" },
          React.createElement("span", { className: "ctf-dot" }),
          "Cuttlefish"
        ),
        React.createElement("div", { className: "ctf-header-actions" },
          React.createElement("button", {
      ...
      // Config
      React.createElement("div", { className: "ctf-config-row" },
        React.createElement("select", {
          className: "ctf-select",
          value: provider,
          onChange: (e: React.ChangeEvent<HTMLSelectElement>) => setProvider(e.target.value as AIProvider),
        },
      ...
      // Prompt
      React.createElement("div", { className: "ctf-prompt-wrap" },
        ...
        React.createElement("div", { className: `ctf-status ${statusText.cls}` }, statusText.text),
        React.createElement("button", {
      ...
      // Actions
      React.createElement("div", { className: "ctf-actions" },
        React.createElement("button", {
      ...
      React.createElement("div", {
        ...
        React.createElement("span", null, selDisplay),
        React.createElement("button", {
      ...
      expanded && React.createElement("div", { className: "ctf-expanded-body" },
        // Tabs
        React.createElement("div", { className: "ctf-tabs" },
          React.createElement("button", {
        ...
        // Patch list
        activeTab === "patches" && React.createElement("div", { className: "ctf-tab-panel" },
          ctx.patches.length === 0
            ? React.createElement("div", { className: "ctf-empty" }, "No patches applied yet")
            : ctx.patches.map((p) =>
                React.createElement("div", { key: p.id, className: "ctf-patch-item" },
                  React.createElement("span", { className: "ctf-patch-op" },  p.op),
                  React.createElement("span", { className: "ctf-patch-target" }, p.target),
                  React.createElement("span", { className: "ctf-patch-src" },   p.source),
                  React.createElement("button", {
                    className: "ctf-patch-rm",
                    onClick: () => ctx.patch({ ...p, op: p.op } as never),
                  }, "✕"),
                )
              )
        ),
        ...
        // Prompt history
        activeTab === "prompts" && React.createElement("div", { className: "ctf-tab-panel" },
          history.length === 0
            ? React.createElement("div", { className: "ctf-empty" }, "No prompts sent yet")
            : history.map((e, i) =>
                React.createElement("div", { key: i, className: "ctf-prompt-item" },
                  React.createElement("div", { className: "ctf-prompt-text" }, e.text),
                  React.createElement("div", { className: "ctf-prompt-meta" },
                    React.createElement("span", null, new Date(e.timestamp).toLocaleTimeString()),
                    e.error
