 ...
 * }
 */
export function useCuttlefish(uid: string): UseCuttlefishReturn {
  ...
  const style = React.useMemo<React.CSSProperties>(() => {
    if (!overrides?.styles) return {};
    const result: Record<string, string> = {};
    for (const [key, value] of Object.entries(overrides.styles)) {
      const camel = key.replace(/-([a-z])/g, (_, l: string) => l.toUpperCase());
      result[camel] = value;
    }
    if (overrides.hidden) result["display"] = "none";
  ...
  function reorder<T extends React.ReactNode>(children: T[]): T[] {
    ...
    for (const child of children) {
      if (React.isValidElement(child)) {
        ...
        const childUid = (props["data-uid"] ?? props["dataUid"]) as string | undefined;
        if (childUid) childMap.set(childUid, child);
      }
    }

    const listedSet = new Set(order);
    const listed    = order.map((u) => childMap.get(u)).filter((c): c is T => c !== undefined);
